...
APP=aisleriot
BIN="sol" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
DEPENDENCES="dconf guile libcanberra librsvg adwaita-cursors" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="guile libvorbis.so $lib_audio_keywords $lib_browser_launcher"
...
_remove_more_bloatwares() {
	...
	find ./"$APP".AppDir/.junest/usr/share/locale/*/*/* -not -iname "*$APP*" -a -not -name "." -delete 2> /dev/null #REMOVE ALL ADDITIONAL LOCALE FILES
